...
class StreamingUITerminal:
    ...
    def start(self):
        ...
        with self.lock:
            ...
            print(
                f"{self.BOLD}{self.GREEN}║ {self.RED}●{self.RESET} {self.BOLD}{'RECORDING - LIVE TRANSCRIPTION'.center(self.width_chars)} {self.GREEN}║{self.RESET}"
            )
            print(f"{self.BOLD}{self.GREEN}{'═' * (self.width_chars + 6)}{self.RESET}")
            print(
                f"{self.GREEN}║{self.RESET} {self.YELLOW}Listening...{self.RESET}".ljust(
                    self.width_chars + 20
                )
    ...
    def update_text(self, text: str):
        ...
        with self.lock:
            ...
            display_text = (
                text[-self.width_chars :] if len(text) > self.width_chars else text
            )
            ...
            if padded != self.last_text:
                ...
                print(
                    f"{self.GREEN}║{self.RESET} {self.BOLD}{padded}{self.RESET} {self.GREEN}║{self.RESET} {self.BLUE}[{self.word_count} words]{self.RESET}",
                    end="",
                    flush=True,
                )
    ...
    def stop(self):
        ...
        with self.lock:
            ...
            print(f"{self.BOLD}{self.GREEN}{'═' * (self.width_chars + 6)}{self.RESET}")
            print(
                f"{self.GREEN}║{self.RESET} {self.BOLD}✓ Recording complete{self.RESET} - {self.BLUE}{self.word_count} words captured{self.RESET}".ljust(
                    self.width_chars + 40
                )
                + f"{self.GREEN}║{self.RESET}"
            )
            print(
                f"{self.BOLD}{self.GREEN}{'═' * (self.width_chars + 6)}{self.RESET}\n"
            )
            sys.stdout.flush()
            self.started = False
...
def create_streaming_ui(
    width_chars: int = 40, prefer_gui: bool = False
) -> StreamingUITerminal:
    """
